...
`;
const NoCourseUpperSpace = styled.div`
  display: block;
  width: 0px;
  height: 0px;
  ...
  padding: 0px;
`;
const NoCourseLowerSpace = styled.div`
  display: block;
  width: 0px;
  height: 0px;
...
function CourseContainer() {
  ...
  return (
    <CourseContainerStyle>
      ...
      {courses.length ? (
        <CourseCards numCourse={numCourse} courses={courses} />
      ) : (
        <NoCourseContainer>
          <NoCourseUpperSpace />
          <NoCourseImg />
          <NoCourseLowerSpace />
          <NoCourseComment>검색 결과가 없습니다.</NoCourseComment>
        </NoCourseContainer>
      )}
    </CourseContainerStyle>
  );
